...
def ensure_today_session_row(database_id: str) -> None:
    ...
    properties = {
        ...
        "Next Step": {"rich_text": rich_text("새 구조를 더 만들지 않고 제목 규칙과 상태 문구를 유지한다.")},
        "Human Summary": {"rich_text": rich_text("노션 운영 표면 stale 정리, session reports 중복 제거, 현재 상태/진행 기록/점검 기록 재동기화와 제목 규칙 정리를 진행 중이다.")},
    ...
    created_id = bootstrap.create_db_row(
        ...
        next_step="새 구조를 더 만들지 않고 제목 규칙과 상태 문구를 유지한다.",
...
def refresh_ops_center(page_id: str) -> None:
    ...
    blocks = [
        ...
        bootstrap.bullet("오늘 세션에서는 stale 현재 상태 표면과 session reports drift를 우선 정리했다."),
...
def refresh_hub(page_id: str) -> None:
    ...
    blocks = [
        ...
        bootstrap.bullet(f"{status_chart}, {progress_chart} chart view는 수동 관리 표면으로 유지"),
